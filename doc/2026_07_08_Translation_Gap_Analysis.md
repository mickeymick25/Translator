...
- Source (nouvelle) : `translator/output/2026_06_26_Export` vs source EN courante
- Langues analysées : ar, cz, de, fr, it, sk, pt, es, hu
...
## Clés à ajouter (communes à toutes les langues : 3)
...
## Clés à modifier — source modifiée entre ancienne et nouvelle source (communes : 3)

| Clé | Nouvelle valeur EN |
|---|---|
| `BU_ST_ME_2169` | Paring Error |
| `LO_AF_CR_1285` | Current Profile |
| `MS_SUCCESS_ERROR_272` | The Location <strong>{{locationId}} - {{locationName}}</strong> links have been updated successfu... |
...
1. **Ajouter** 3 nouvelle(s) clé(s) × 9 langue(s) = 27 traductions à produire.
2. **Supprimer** 0 clé(s) orpheline(s) des fichiers cibles (commun à toutes les langues).
3. **Modifier** 3 clé(s) dont la source a changé (retraduire).
