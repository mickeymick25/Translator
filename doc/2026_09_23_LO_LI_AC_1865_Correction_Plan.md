...
`15eaa4b`, ex-`b61f0aa`). Aucune correction prévue : si une fusion CSV de référence était
...
La fusion 06_12 était ad hoc (scripts supprimés dans `15eaa4b`, ex-`b61f0aa`) ; le pipeline
...
| 2026-09-23 | **FIX-1865-03** | Commit `8649ae4` ex-`bf38039` sur branche `fix/FIX-1865-03-detecteur-duplication` (hooks OK : lint + 873 tests conteneur ; correction Dockerfile préexistante : COPY validate_translations.py) — **contrainte : pas de push vers origin, travail local uniquement** | Michael |
| 2026-09-23 | **FIX-1865-03** | ✅ Squash-merge vers `main` (`28e254a` ex-`2571ece`) — branche locale conservée | Agent |
...
| 2026-09-23 | — | 🧹 **Purge d'historique** (décision Michael, option B + complément « output vide ») : 2 passes `git filter-repo` (Docker, python:3.11-slim) — passe 1 : `Export_COP_MessageExcel.numbers` (fuite 17/06) + `translation_en_fr.json.bak` ; passe 2 : artefacts recette sous output (TSV mapping officiel, checklist, recette) + `doc/2026_06_26_FR_Misalignment_History.csv` — output vide de contenu dans tout l'historique ; backup pré-purge : `translator_backup_20260923.bundle` (racine projet). Remappage final : bf38039→334dc5e · 1d7237d→d9c8efa · 9244a20→b25720b · 2571ece→e2784ab · 7f41a4c→3c78ae2 · 8fefea9→**commit supprimé (vidé par la purge)** · f40f4ae→c197d11 · b61f0aa→d5f7feb · 1614272→0824554 · c8ef638→075310c · 393bb87→ee9ba8a · 27f116c→7c474ee · f3d507e→34c6b54 · cf49e5e→d461346 — puis **force-push requis** | Agent |
