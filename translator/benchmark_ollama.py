#!/usr/bin/env python3
"""
Mini-benchmark Ollama — Compare des modèles sur un échantillon de traduction.

Usage:
    python3 benchmark_ollama.py

Teste 3 modèles (glm-5.1:cloud, qwen3, minimax-m2.7:cloud)
sur 50 entrées, en FR et CZ, et mesure :
  - Temps total et par chunk
  - Taux de JSON invalide
  - Taux de corruption placeholders
  - Taux de fallback (valeurs originales conservées)
  - Complétude (% clés retournées)

Résultats affichés en tableau comparatif.
"""
...
SOURCE_FILE = Path(__file__).parent / "source" / "2026_05_27_Import" / "en 5.json"
SAMPLE_SIZE = 30
CHUNK_SIZE = 20
TEMPERATURE = 0
MAX_RETRIES = 2
...
def benchmark_model(
    ...
    print(f"\n  🔄 {model} → {target_name} ({len(sample)} entries)...")
...
def main():
    ...
    for model in MODELS:
        ...
        for source_lang, target_lang, target_name in LANGUAGES:
            result = benchmark_model(
                model, source_data, sample, source_lang, target_lang, target_name
            )
    ...
    print("=" * 90)
    print(
        f"{'Model':<22} {'Lang':<6} {'Time':>6} {'JSON':>6} "
    ...
    for r in results:
        ...
        print(
            f"{r['model']:<22} {r['lang']:<6} {r['time']:>5.1f}s {json_mark:>4} "
